...
void testHash() {
    ...
    unsigned char bin[5] = "0101", bin1[5] = "0111", bin2[5] = "0001";
    put_in_hash(ht, 'a', bin, 4);
    CU_ASSERT_STRING_EQUAL(ht->table['a']->binary, bin);

    put_in_hash(ht, 'b', bin, 8);
    CU_ASSERT_STRING_EQUAL(ht->table['b']->binary, bin);

    put_in_hash(ht, 'f', bin, 89);
    CU_ASSERT_STRING_EQUAL(ht->table['f']->binary, bin);
